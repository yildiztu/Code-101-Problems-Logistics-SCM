...
def main():
    ...
    header_files = [
        r"10. The Dual-Cycling Quay Crane Problem\executed\P10-Tier-1.ipynb",
        r"11. The Dynamic Berth Allocation Problem\executed\P11-Tier-1.ipynb",
        r"12. The Berth Allocation with Tidal Windows Problem\executed\P12-Tier-1.ipynb",
        r"12. The Berth Allocation with Tidal Windows Problem\executed\P12-Tier-7.ipynb"
    ]
    ...
    for rel_path in header_files:
        ...
        if os.path.exists(filepath):
            # We look for "Tier X:" and replace with "Tier X —"
            # Since the number X varies, we might need a generic approach or specific calls.
            # Based on previous grep, these specific files have matches.
            # I will use a generic replacement logic inside the function if needed, 
            # but for now let's try specific replacements if we know them, or regex?
            # The simple replace string function above expects exact string.
            # I'll modify the loop to read the file and check what Tier number it is.
            pass # Logic moved to specific block below
        else:
