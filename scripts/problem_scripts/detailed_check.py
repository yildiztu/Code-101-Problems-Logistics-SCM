...
def detailed_notebook_check(notebook_path):
    ...
    try:
        ...
        empty_code = [c for c in code_cells if not c.get('source', '') or (isinstance(c.get('source', ''), list) and not ''.join(c.get('source', '')).strip())]
        empty_markdown = [c for c in markdown_cells if not c.get('source', '') or (isinstance(c.get('source', ''), list) and not ''.join(c.get('source', '')).strip())]
